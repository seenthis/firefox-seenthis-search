...
function finalizeWindow(aWindow) {
  ...
  for (var i = 0, len = tabs.length; i < len; i++) {
    if (tabs[i].linkedBrowser.currentURI.spec.indexOf('chrome://seenthis_search/') == 0) {
...
// toolbar
const TOOLBAR_BUTTON_ELEMENT_ID = "seenthisIncsearchButton";
...
function initializeToolbar(aWindow) {
  ...
  toolbarbutton.setAttribute("label", "seenthis.net search");
  toolbarbutton.setAttribute("tooltiptext", "seenthis.net search");
  toolbarbutton.style.listStyleImage = 'url("chrome://seenthis_search/skin/icon-small.png")';
...
// shortcut key
const SHORTCUT_KEY_ELEMENT_ID = "seenthisSearchShortcutKey";
const SHORTCUT_KEYSET_ELEMENT_ID = "seenthisSearchShortcutKeySet";

function initializeShortcutKey(aWindow) {

  var prefBranch = PrefService.getBranch("extensions.seenthis_search.");
...
// menu
const MENU_ITEM_ELEMENT_ID = "seenthisSearchMenuitem"
...
function initializeMenu(aWindow) {
  ...
  menuitem.setAttribute("label", "seenthis.net search");
